...
// TestDiff_IgnoredFields_StillRecorded: bookkeeping is a read-side concern —
// the option must not change what Diff records. The changelog is the data
// record; hiding happens on Explain (see TestExplain_BookkeepingFlag).
func TestDiff_IgnoredFields_StillRecorded(t *testing.T) {
	...
	flagged, err := Diff(before, after, WithIgnoredFields("updated_at", "rev"))
	if err != nil {
		t.Fatalf("diff: %v", err)
	}
	if len(plain) != 2 || len(flagged) != len(plain) {
		t.Fatalf("option must not affect recording: plain=%d flagged=%d", len(plain), len(flagged))
	}
}
